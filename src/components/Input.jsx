...
export default function Input(props) {
    ...
    const handleChange = (e) => {
        ...
        if (time) {
            ...
            if (val.length > 4) val = val.slice(0, 4);   // 최대 4자리
    ...
    return (
        <div className={`input_box ${size ? 'input_box_'+size : ''} ${error ? 'error' : ''} ${setCSS ? setCSS : ''}`} style={style}>
            {label && 
            ...
            <input 
                ...
                name={name} 
                id={name} 
                className={`input-text size-lg ${className + (readOnly ? " disable" : "") + (withButton ? " withButton" : "") + (withText ? " withText" : "") + (search ? ' withSearch' : '')}`} 
